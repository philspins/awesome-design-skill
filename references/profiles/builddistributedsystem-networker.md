...
**Audience**: Software engineers, platform engineers, and architects learning distributed systems networking concepts.

---

## 1) Visual Theme & Atmosphere

The Networker track is a technical learning platform for distributed systems. Its visual language draws from networking topology diagrams, infrastructure dashboards (Grafana, Datadog), and dark developer tooling. The atmosphere is energetic yet precise — dark surfaces with neon-accented connectivity visualisations communicate that networking is both scientific and alive.

- **Product context**: Online track/learning module for distributed systems engineers; sessions displayed as interconnected nodes.
- **Brand mood keywords**: technical, connected, nocturnal, precise, energetic, diagrammatic
- **Density strategy**: balanced — generous breathing room around card content, but rich with iconography and data in session/topology panels
- **Personality contrast**: cinematic-dark + developer-approachable; dramatic contrast without being hostile
...
| `accent.primary` | `#00D4FF` | Teal — primary interactive, links, CTA, active node glow |
| `accent.secondary` | `#7C3AED` | Violet — secondary brand color, session category badges |
| `accent.highlight` | `#F97316` | Orange — highlights, call-to-action emphasis, warning |
| `accent.node` | `#10B981` | Green — active connection / live status indicators |
| `border.default` | `#1E3A5F` | Standard borders |
| `border.glow` | `rgba(0,212,255,0.35)` | Neon glow border on focus / hover |
| `success` | `#10B981` | Success, connected, nominal |
| `warning` | `#F97316` | Degraded, latency spike |
| `error` | `#EF4444` | Error, unreachable, failed |
...
- **Display family**: `"Inter"` (semibold/bold headings) — clean geometric sans-serif optimised for technical content at large sizes
- **Body family**: `"Inter"`, fallback `system-ui, -apple-system, sans-serif`
- **Monospace family**: `"JetBrains Mono"`, fallback `"Fira Code", "Cascadia Code", monospace` — used for code, hostnames, IP addresses, protocol labels, CLI examples
...
| Label / Badge | 11px | 600 | 1 | Track badges, status labels (uppercase) |

**Rules**:
- All protocol names, endpoint paths, hostnames, and packet fields use the mono stack.
- Heading color defaults to `fg.primary`; interactive headings may use `accent.primary` with a hover state.
- Never italicise headings; reserve italics for inline technical asides only.
...
|---|---|---|---|---|---|
| Primary | `accent.primary #00D4FF` | none | `bg.base #0B0E1A` | 6px | Enroll CTA, main action |
...
- Node badge (top-left corner): small circle `12px` in `accent.node` green with a pulsing ring animation when "live"
- Session cards display: track badge (violet), title (H3), speaker avatar + name, duration in mono
...
- Row hover: `bg.subtle`
- Numeric / address columns: JetBrains Mono 14px
...
- **Grid model**: 12-column CSS Grid; gutter 24px; track pages use a 2-up card grid on wide, single column on mobile
- **Container widths**:
  - Max content width: `1280px`
  - Narrow editorial: `768px` (long-form descriptions, session details)
  - Full-bleed: hero sections and topology diagram backgrounds only
...
- **Rhythm / whitespace rules**:
  ...
  - Topology diagram sections may bleed to full viewport width; constrain text overlay to `768px`
...
| Glow | — | `border.glow` | `0 0 20px rgba(0,212,255,0.2)` | Active node indicators, CTA hover |

**Blur / glass rules**:
- Frosted glass (`backdrop-filter: blur(12px)`) is allowed only for the sticky nav bar and modal overlays; never on content cards.
- Decorative topology background layers may use `opacity: 0.08–0.15` SVG pattern overlays (dot grid or hexagonal mesh) — never full-opacity graphics behind text.
...
**Do**
- Use `JetBrains Mono` for all protocol names, IPs, port numbers, hostnames, and CLI examples.
- Represent session/content connections with node-and-edge metaphors (cards as nodes, lines as relationships).
- Apply teal glow exclusively to primary interactive elements — it is the brand's interactive signal.
...
- Animate only with purpose: pulsing node rings for "live" status, subtle card lift on hover, smooth color transitions (150–200ms ease).
- Keep background topology/diagram graphics at very low opacity (≤0.12) — they are atmospheric, not informational.

**Don't**
- Do not use light mode — this system is dark-only; no light theme.
- Do not use more than two simultaneous glow effects on a single screen region.
- Do not use `accent.highlight` (orange) for anything other than emphasis / warnings — never use it as a primary brand color.
- Do not use thin (< 400 weight) text below 14px on dark surfaces — legibility degrades.
- Do not add decorative gradients to text (gradient headings) — reserve gradients for background surfaces only.
- Do not use the violet `accent.secondary` for primary CTAs — it competes with the teal brand signal.
...
- **Layout collapse behavior**:
  ...
  - Topology diagram sections stack vertically; diagram moves above text on mobile
...
> Build a page using the BuildDistributedSystem Networker aesthetic. Dark base `#0B0E1A`, surface cards `#111827`, elevated panels `#1A2235`. Primary interactive teal `#00D4FF` with glow hover (`box-shadow: 0 0 12px rgba(0,212,255,0.45)`). Violet `#7C3AED` for category badges. Orange `#F97316` for warnings and emphasis only. Inter for all text; JetBrains Mono for all code, IPs, protocols, and technical labels. H1 36px/700, body 16px/400, mono 14px/400. Cards: 10px radius, `1px solid #1E3A5F` border, 24px padding, hover lifts 2px with glow border. Nav: sticky 64px, dark bg, teal active underline. Spacing 8px base. 12-column grid, 1280px max-width. Focus ring: `2px solid #00D4FF, offset 2px`. No light mode. No decorative gradients on text. Status indicators always pair color with label.

**Negative prompt (what to avoid):**
- No light mode or light-colored backgrounds
- No gradient text effects
- No multiple stacked glows on a single element
- No use of orange as a primary/brand color (warnings only)
- No color-only status indicators
- No rounded corners > 12px (cards) or > 6px (buttons/inputs)
- No large hero images replacing diagram/topology visuals
...
- [ ] Code, IPs, protocols, and technical strings use JetBrains Mono
...
- [ ] Orange is used only for warnings or accent emphasis, never as primary CTA
- [ ] Touch targets ≥ 44px on all interactive controls
- [ ] Background topology graphics are ≤ 12% opacity
